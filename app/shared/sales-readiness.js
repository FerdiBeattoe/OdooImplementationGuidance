import { getSalesConfigurationSections } from "./sales-configuration.js";
import { SAMPLE_GUIDANCE_BLOCKS } from "./guidance.js";

const READY_BLOCKING_CLASSES = new Set(["Foundational", "Domain Required", "Go-Live"]);

export function getSalesReadinessSummary(project) {
  const checkpoints = Array.isArray(project?.checkpoints)
    ? project.checkpoints.filter((checkpoint) => checkpoint?.domainId === "sales")
    : [];

  if (!checkpoints.length) {
    return {
      readinessState: "not_ready",
      explanation: "Sales checkpoint state is unavailable. Do not claim readiness.",
      blockerCount: 0,
      blockers: [],
      failedCheckpoints: [],
      evidenceGaps: [],
      ...
      nextActions: [
        {
          priority: "blocker",
          checkpointId: "",
          text: "Review Sales checkpoint state before making readiness claims."
        }
      ],
      captureContext: [],
  ...
  const configurationSections = getSalesConfigurationSections(project || {});
  ...
  const evidenceGaps = checkpoints
    ...
    .map((checkpoint) => ({
      checkpointId: checkpoint.id,
      title: checkpoint.title,
      evidenceMode: checkpoint.salesEvidence?.mode || "none",
      evidenceLabel: getEvidenceGapLabel(checkpoint)
    }));
  const captureOnly = checkpoints
    .filter((checkpoint) => checkpoint.salesEvidence?.mode === "design_capture")
    .map((checkpoint) => ({
      checkpointId: checkpoint.id,
      title: checkpoint.title,
      summary: checkpoint.salesEvidence?.summary || "Design capture exists without stronger accountable support."
    }));
  ...
  const hasStrongEvidenceOnly = checkpoints.every((checkpoint) => checkpoint.salesEvidence?.mode === "system_detected");

  let readinessState = "not_ready";
  let explanation =
    "Sales readiness is not yet proven. Review blockers, accountable support, and downstream impacts before treating this slice as dependable.";

  if (requiredFailures.length > 0) {
    readinessState = "not_ready";
    explanation = "Sales readiness remains blocked by failed required or go-live checkpoints.";
  } else if (criticalEvidenceGaps.length > 0) {
    readinessState = "not_ready";
    explanation = "Sales checkpoints are still supported only by missing or capture-only evidence on required paths.";
  } else if (allCheckpointsPassed && !hasEvidenceGaps && hasStrongEvidenceOnly) {
    readinessState = "ready";
    explanation = "Sales checkpoint and evidence state support a strict readiness claim.";
  } else if (requiredPassed) {
    readinessState = "conditionally_ready";
    explanation =
      "Sales blockers are cleared, but readiness is still limited. Remaining support is not system-proven and must not be treated as hard operational readiness.";
  }
...
function getEvidenceGapLabel(checkpoint) {
  const mode = checkpoint.salesEvidence?.mode || "none";
