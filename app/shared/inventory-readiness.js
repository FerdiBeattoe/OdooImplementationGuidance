...
import { getInventoryConfigurationSections } from "./inventory-configuration.js";
import { getInventoryEvidenceSufficiency } from "./inventory-evidence.js";

const READY_BLOCKING_CLASSES = new Set(["Foundational", "Domain Required", "Go-Live"]);

export function getInventoryReadinessSummary(project) {
  const checkpoints = Array.isArray(project?.checkpoints)
    ? project.checkpoints.filter((checkpoint) => checkpoint?.domainId === "inventory")
    : [];

  if (!checkpoints.length) {
    return {
      readinessState: "not_ready",
      explanation: "Inventory checkpoint state is unavailable. Do not claim readiness.",
      blockerCount: 0,
      blockers: [],
      failedCheckpoints: [],
      deferredCheckpoints: [],
      ...
      nextActions: [
        {
          priority: "blocker",
          checkpointId: "",
          text: "Review Inventory checkpoint state before making readiness claims."
        }
      ],
      captureContext: [],
  ...
  const configurationSections = getInventoryConfigurationSections(project || {});
  ...
  const evidenceGaps = checkpoints
    ...
    .map((checkpoint) => ({
      checkpointId: checkpoint.id,
      title: checkpoint.title,
      evidenceMode: checkpoint.inventoryEvidence?.mode || "none",
      sufficiency: getInventoryEvidenceSufficiency(checkpoint)
    }));
  const captureOnly = checkpoints
    .filter((checkpoint) => checkpoint.inventoryEvidence?.mode === "design_capture")
    .map((checkpoint) => ({
      checkpointId: checkpoint.id,
      title: checkpoint.title,
      summary: checkpoint.inventoryEvidence?.summary || "Design capture exists without stronger proof."
    }));
  ...
  const requiredOrGoLiveFailures = failedCheckpoints.filter((checkpoint) => READY_BLOCKING_CLASSES.has(checkpoint.checkpointClass));
  const requiredOrGoLiveCheckpoints = checkpoints.filter((checkpoint) => READY_BLOCKING_CLASSES.has(checkpoint.checkpointClass));
  const requiredOrGoLivePassed = requiredOrGoLiveCheckpoints.every((checkpoint) => checkpoint.status === "Pass");
  const allInventoryPassed = checkpoints.every((checkpoint) => checkpoint.status === "Pass");
  const hasDeferred = deferredCheckpoints.length > 0;
  const hasEvidenceGaps = evidenceGaps.length > 0;
  const hasStrongEvidenceOnly = checkpoints.every((checkpoint) => checkpoint.inventoryEvidence?.mode === "system_detected");

  let readinessState = "not_ready";
  let explanation = "Inventory is not yet proven ready. Review blockers, evidence, and deferred conditions before claiming progress.";

  if (requiredOrGoLiveFailures.length > 0) {
    readinessState = "not_ready";
    explanation = "Inventory remains blocked by failed required or go-live checkpoints.";
  } else if (criticalEvidenceGaps.length > 0) {
    readinessState = "not_ready";
    explanation = "Inventory required/go-live checkpoints are not yet supported by strong enough evidence.";
  } else if (allInventoryPassed && !hasDeferred && !hasEvidenceGaps && hasStrongEvidenceOnly) {
    readinessState = "ready";
    explanation = "Inventory checkpoint and evidence state support a strict readiness claim.";
  } else if (requiredOrGoLivePassed && hasDeferred && criticalEvidenceGaps.length === 0) {
    readinessState = "conditionally_ready";
    explanation = "Inventory required/go-live checkpoints pass, but readiness is still not proven. Deferred items and weaker non-system evidence require explicit follow-up and review.";
  }
...
function isEvidenceGap(checkpoint) {
  const mode = checkpoint.inventoryEvidence?.mode || "none";
