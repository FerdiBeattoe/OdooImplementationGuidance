...
import { getWebsiteEcommerceConfigurationSections } from "./website-ecommerce-configuration.js";
import { getWebsiteEcommerceEvidenceSufficiency } from "./website-ecommerce-evidence.js";

const READY_BLOCKING_CLASSES = new Set(["Foundational", "Domain Required", "Go-Live"]);

export function getWebsiteEcommerceReadinessSummary(project) {
  const checkpoints = Array.isArray(project?.checkpoints)
    ? project.checkpoints.filter((checkpoint) => checkpoint?.domainId === "website-ecommerce")
    : [];

  if (!checkpoints.length) {
    return {
      readinessState: "not_ready",
      explanation: "Website/eCommerce checkpoint state is unavailable. Do not claim readiness.",
      blockerCount: 0,
      blockers: [],
      failedCheckpoints: [],
      deferredCheckpoints: [],
      ...
      nextActions: [
        {
          priority: "blocker",
          checkpointId: "",
          text: "Review Website/eCommerce checkpoint state before making readiness claims."
        }
      ],
      captureContext: [],
  ...
  const configurationSections = getWebsiteEcommerceConfigurationSections(project || {});
  ...
  const evidenceGaps = checkpoints
    ...
    .map((checkpoint) => ({
      checkpointId: checkpoint.id,
      title: checkpoint.title,
      evidenceMode: checkpoint.websiteEcommerceEvidence?.mode || "none",
      sufficiency: getWebsiteEcommerceEvidenceSufficiency(checkpoint)
    }));
  const captureOnly = checkpoints
    .filter((checkpoint) => checkpoint.websiteEcommerceEvidence?.mode === "design_capture")
    .map((checkpoint) => ({
      checkpointId: checkpoint.id,
      title: checkpoint.title,
      summary: checkpoint.websiteEcommerceEvidence?.summary || "Design capture exists without stronger proof."
    }));
  ...
  const allWebsitePassed = checkpoints.every((checkpoint) => checkpoint.status === "Pass");
  ...
  const hasStrongEvidenceOnly = checkpoints.every(
    (checkpoint) => checkpoint.websiteEcommerceEvidence?.mode === "system_detected"
  );

  let readinessState = "not_ready";
  let explanation =
    "Website/eCommerce is not yet proven ready. Review blockers, evidence, and deferred conditions before claiming progress.";

  if (requiredOrGoLiveFailures.length > 0) {
    readinessState = "not_ready";
    explanation = "Website/eCommerce remains blocked by failed required or go-live checkpoints.";
  } else if (criticalEvidenceGaps.length > 0) {
    readinessState = "not_ready";
    explanation = "Website/eCommerce required/go-live checkpoints are not yet supported by strong enough evidence.";
  } else if (allWebsitePassed && !hasDeferred && !hasEvidenceGaps && hasStrongEvidenceOnly) {
    readinessState = "ready";
    explanation = "Website/eCommerce checkpoint and evidence state support a strict readiness claim.";
  } else if (requiredOrGoLivePassed && hasDeferred && criticalEvidenceGaps.length === 0) {
    readinessState = "conditionally_ready";
    explanation =
      "Website/eCommerce required/go-live checkpoints pass, but readiness is still not proven. Deferred items and weaker non-system evidence require explicit follow-up and review.";
  }
...
function isEvidenceGap(checkpoint) {
  const mode = checkpoint.websiteEcommerceEvidence?.mode || "none";
