...
import { getCrmConfigurationSections } from "./crm-configuration.js";
import { getCrmEvidenceSufficiency } from "./crm-evidence.js";

const READY_BLOCKING_CLASSES = new Set(["Foundational", "Domain Required", "Go-Live"]);

export function getCrmReadinessSummary(project) {
  const checkpoints = Array.isArray(project?.checkpoints)
    ? project.checkpoints.filter((checkpoint) => checkpoint?.domainId === "crm")
    : [];

  if (!checkpoints.length) {
    return {
      readinessState: "not_ready",
      explanation: "CRM checkpoint state is unavailable. Do not claim readiness.",
      blockerCount: 0,
      blockers: [],
      failedCheckpoints: [],
      deferredCheckpoints: [],
      ...
      nextActions: [
        {
          priority: "blocker",
          checkpointId: "",
          text: "Review CRM checkpoint state before making readiness claims."
        }
      ],
      captureContext: [],
  ...
  const configurationSections = getCrmConfigurationSections(project || {});
  ...
  const evidenceGaps = checkpoints
    ...
    .map((checkpoint) => ({
      checkpointId: checkpoint.id,
      title: checkpoint.title,
      evidenceMode: checkpoint.crmEvidence?.mode || "none",
      sufficiency: getCrmEvidenceSufficiency(checkpoint)
    }));
  const captureOnly = checkpoints
    .filter((checkpoint) => checkpoint.crmEvidence?.mode === "design_capture")
    .map((checkpoint) => ({
      checkpointId: checkpoint.id,
      title: checkpoint.title,
      summary: checkpoint.crmEvidence?.summary || "Design capture exists without stronger proof."
    }));
  ...
  const allCrmPassed = checkpoints.every((checkpoint) => checkpoint.status === "Pass");
  ...
  const hasStrongEvidenceOnly = checkpoints.every(
    (checkpoint) => checkpoint.crmEvidence?.mode === "system_detected"
  );

  let readinessState = "not_ready";
  let explanation =
    "CRM is not yet proven ready. Review blockers, evidence, and deferred conditions before claiming progress.";

  if (requiredOrGoLiveFailures.length > 0) {
    readinessState = "not_ready";
    explanation = "CRM remains blocked by failed required or go-live checkpoints.";
  } else if (criticalEvidenceGaps.length > 0) {
    readinessState = "not_ready";
    explanation = "CRM required/go-live checkpoints are not yet supported by strong enough evidence.";
  } else if (allCrmPassed && !hasDeferred && !hasEvidenceGaps && hasStrongEvidenceOnly) {
    readinessState = "ready";
    explanation = "CRM checkpoint and evidence state support a strict readiness claim.";
  } else if (requiredOrGoLivePassed && hasDeferred && criticalEvidenceGaps.length === 0) {
    readinessState = "conditionally_ready";
    explanation =
      "CRM required/go-live checkpoints pass, but readiness is still not proven. Deferred items and weaker non-system evidence require explicit follow-up and review.";
  }
...
function isEvidenceGap(checkpoint) {
  const mode = checkpoint.crmEvidence?.mode || "none";
