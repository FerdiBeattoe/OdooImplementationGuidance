...
import { getManufacturingConfigurationSections } from "./manufacturing-configuration.js";
import { getManufacturingEvidenceSufficiency } from "./manufacturing-evidence.js";

const READY_BLOCKING_CLASSES = new Set(["Foundational", "Domain Required", "Go-Live"]);

export function getManufacturingReadinessSummary(project) {
  const checkpoints = Array.isArray(project?.checkpoints)
    ? project.checkpoints.filter((checkpoint) => checkpoint?.domainId === "manufacturing-mrp")
    : [];

  if (!checkpoints.length) {
    return {
      readinessState: "not_ready",
      explanation: "Manufacturing checkpoint state is unavailable. Do not claim readiness.",
      blockerCount: 0,
      blockers: [],
      failedCheckpoints: [],
      deferredCheckpoints: [],
      ...
      nextActions: [
        {
          priority: "blocker",
          checkpointId: "",
          text: "Review Manufacturing checkpoint state before making readiness claims."
        }
      ],
      captureContext: [],
  ...
  const configurationSections = getManufacturingConfigurationSections(project || {});
  ...
  const evidenceGaps = checkpoints
    ...
    .map((checkpoint) => ({
      checkpointId: checkpoint.id,
      title: checkpoint.title,
      evidenceMode: checkpoint.manufacturingEvidence?.mode || "none",
      sufficiency: getManufacturingEvidenceSufficiency(checkpoint)
    }));
  const captureOnly = checkpoints
    .filter((checkpoint) => checkpoint.manufacturingEvidence?.mode === "design_capture")
    .map((checkpoint) => ({
      checkpointId: checkpoint.id,
      title: checkpoint.title,
      summary: checkpoint.manufacturingEvidence?.summary || "Design capture exists without stronger proof."
    }));
  ...
  const allManufacturingPassed = checkpoints.every((checkpoint) => checkpoint.status === "Pass");
  ...
  const hasStrongEvidenceOnly = checkpoints.every(
    (checkpoint) => checkpoint.manufacturingEvidence?.mode === "system_detected"
  );

  let readinessState = "not_ready";
  let explanation =
    "Manufacturing is not yet proven ready. Review blockers, evidence, and deferred conditions before claiming progress.";

  if (requiredOrGoLiveFailures.length > 0) {
    readinessState = "not_ready";
    explanation = "Manufacturing remains blocked by failed required or go-live checkpoints.";
  } else if (criticalEvidenceGaps.length > 0) {
    readinessState = "not_ready";
    explanation = "Manufacturing required/go-live checkpoints are not yet supported by strong enough evidence.";
  } else if (allManufacturingPassed && !hasDeferred && !hasEvidenceGaps && hasStrongEvidenceOnly) {
    readinessState = "ready";
    explanation = "Manufacturing checkpoint and evidence state support a strict readiness claim.";
  } else if (requiredOrGoLivePassed && hasDeferred && criticalEvidenceGaps.length === 0) {
    readinessState = "conditionally_ready";
    explanation =
      "Manufacturing required/go-live checkpoints pass, but readiness is still not proven. Deferred items and weaker non-system evidence require explicit follow-up and review.";
  }
...
function isEvidenceGap(checkpoint) {
  const mode = checkpoint.manufacturingEvidence?.mode || "none";
